...
public class UnsafeMpMcRingBuffer<T> extends AbstractMpMcRingBuffer<T> {
    ...
    public UnsafeMpMcRingBuffer(int capacity, boolean bufferPaddingEnabled, IntFunction<T> elementInstanceProducer) {
    ...
    protected T getBufferElement(int index) {
        return (T) UNSAFE.getReference(buffer, bufferOffset(index));
    }
    ...
    protected void setBufferElement(int index, T value) {
        UNSAFE.putReference(buffer, bufferOffset(index), value);
    }
