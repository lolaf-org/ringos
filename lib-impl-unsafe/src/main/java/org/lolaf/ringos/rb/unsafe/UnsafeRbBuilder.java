...
public class UnsafeRbBuilder implements RingBufferBuilder {
        ...
        try {
            switch (accessType) {
                case SINGLE_CONSUMER_SINGLE_PRODUCER:
                    return new UnsafeSpScRingBuffer<>(capacity, bufferPaddingEnabled, elementInstanceProducer);
                case SINGLE_CONSUMER_MULTI_PRODUCER:
                    return elementInstanceProducer == null
                            ? new UnsafeMpScRingBuffer<>(capacity, bufferPaddingEnabled)
                            : new UnsafePooledMpScRingBuffer<>(capacity, bufferPaddingEnabled, elementInstanceProducer);
                case MULTI_CONSUMER_SINGLE_PRODUCER:
                    return new UnsafeSpMcRingBuffer<>(capacity, bufferPaddingEnabled, elementInstanceProducer);
                case MULTI_CONSUMER_MULTI_PRODUCER:
                    return new UnsafeMpMcRingBuffer<>(capacity, bufferPaddingEnabled, elementInstanceProducer);
                default:
