...
public class MethodHandleRbBuilder implements RingBufferBuilder {
    ...
    public <T> RingBuffer<T> build(RingBufferFactory.AccessType accessType, int capacity, boolean bufferPaddingEnabled, IntFunction<T> elementInstanceProducer) {
        switch (accessType) {
            case SINGLE_CONSUMER_SINGLE_PRODUCER:
                return new MethodHandleSpScRingBuffer<>(capacity, bufferPaddingEnabled, elementInstanceProducer);
            case SINGLE_CONSUMER_MULTI_PRODUCER:
                return elementInstanceProducer == null
                        ? new MethodHandleMpScRingBuffer<>(capacity, bufferPaddingEnabled)
                        : new MethodHandlePooledMpScRingBuffer<>(capacity, bufferPaddingEnabled, elementInstanceProducer);
            case MULTI_CONSUMER_SINGLE_PRODUCER:
                return new MethodHandleSpMcRingBuffer<>(capacity, bufferPaddingEnabled, elementInstanceProducer);
            case MULTI_CONSUMER_MULTI_PRODUCER:
                return new MethodHandleMpMcRingBuffer<>(capacity, bufferPaddingEnabled, elementInstanceProducer);
            default:
