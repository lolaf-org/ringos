...
public abstract class AbstractMpMcRingBuffer<T> extends AbstractSequencedRingBuffer<T> {

    protected AbstractMpMcRingBuffer(int capacity, boolean bufferPaddingEnabled) {
        this(capacity, bufferPaddingEnabled, null);
    }

    protected AbstractMpMcRingBuffer(int capacity, boolean bufferPaddingEnabled, IntFunction<T> elementInstanceProducer) {
    ...
    public boolean offer(T item) {
        ...
        do {
            ...
            if (tailCompareAndSwap(currentTail, nextTail)) {
    ...
    public T poll() {
        do {
            ...
            if (headCompareAndSwap(currentHead, nextHead)) {
    ...
    public int poll(Consumer<T>[] consumers) {
        do {
            ...
            long targetHead = currentHead + available;
            if (headCompareAndSwap(currentHead, targetHead)) {
    ...
    protected abstract boolean headCompareAndSwap(long current, long next);
