...
public class J11UnsafeMpMcRingBuffer<T> extends AbstractMpMcRingBuffer<T> {
    ...
    public J11UnsafeMpMcRingBuffer(int capacity, boolean bufferPaddingEnabled, IntFunction<T> elementInstanceProducer) {
    ...
    protected T getBufferElement(int index) {
        return (T) UNSAFE.getObject(buffer, bufferOffset(index));
    }
    ...
    protected void setBufferElement(int index, T value) {
        UNSAFE.putObject(buffer, bufferOffset(index), value);
    }
